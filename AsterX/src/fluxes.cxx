...
void CalcFlux(CCTK_ARGUMENTS, EOSType *eos_3p, const rec_var_t rec_var,
  ...
  grid.loop_mixpn_device<
    ...
    // Radiation prefactors at the two neighboring cell centers; the EOS
    // depends on radiation solely through these scalars.
    const CCTK_REAL rad_pref_rc[2] = {
        eos_rad_prefactor(eos_3p,
                          local_optd<EOSType>(od_gfs, p.I - p.DI[dir_i]),
                          rad_ramp),
        eos_rad_prefactor(eos_3p, local_optd<EOSType>(od_gfs, p.I),
                          rad_ramp)};

    const auto press_from_rho_temp =
        [&](const CCTK_REAL rho_, const CCTK_REAL temp_,
	            const CCTK_REAL ye_, const CCTK_REAL pref_) ARITH_INLINE {
	          return eos_press_from_rho_temp(eos_3p, rho_, temp_, ye_, pref_);
	        };
    const auto eps_from_rho_temp =
        [&](const CCTK_REAL rho_, const CCTK_REAL temp_,
	            const CCTK_REAL ye_, const CCTK_REAL pref_) ARITH_INLINE {
	          return eos_eps_from_rho_temp(eos_3p, rho_, temp_, ye_, pref_);
	        };
    const auto eps_from_rho_press =
        [&](const CCTK_REAL rho_, const CCTK_REAL press_,
	            const CCTK_REAL ye_, const CCTK_REAL pref_) ARITH_INLINE {
	          return eos_eps_from_rho_press(eos_3p, rho_, press_, ye_, pref_);
	        };
    const auto temp_from_rho_eps =
        [&](const CCTK_REAL rho_, CCTK_REAL &eps_, const CCTK_REAL ye_,
	            const CCTK_REAL pref_) ARITH_INLINE {
	          return eos_temp_from_rho_eps(eos_3p, rho_, eps_, ye_, pref_);
	        };
    ...
    const auto csnd_from_rho_temp =
        [&](const CCTK_REAL rho_, const CCTK_REAL temp_, const CCTK_REAL ye_,
	            const CCTK_REAL pref_) ARITH_INLINE {
	          return eos_csnd_from_rho_temp(eos_3p, rho_, temp_, ye_, pref_);
	        };
    ...
    if (use_press_atmo) {
      press_atm(0) = (r_atm(0) > r_atmo)
                         ...
                         : p_atmo;
      press_atm(0) = std::max(
          eos_3p->press_from_rho_temp_ye(rho_atm(0), eos_3p->rgtemp.min,
                                         Ye_atmo),
          press_atm(0));
      press_atm(1) = (r_atm(1) > r_atmo)
                         ...
                         : p_atmo;
      press_atm(1) = std::max(
          eos_3p->press_from_rho_temp_ye(rho_atm(1), eos_3p->rgtemp.min,
                                         Ye_atmo),
          press_atm(1));
      eps_atm(0) =
    ...
    if (reconstruct_with_temperature) {
      ...
      for (int f = 0; f < 2; ++f) {
        eps_rc(f) =
            eps_from_rho_temp(rho_rc(f), temp_rc(f), Ye_rc(f), rad_pref_rc[f]);
        press_rc(f) =
            press_from_rho_temp(rho_rc(f), temp_rc(f), Ye_rc(f), rad_pref_rc[f]);
      }
      ...
      for (int f = 0; f < 2; ++f) {
        eps_rc(f) =
            eps_from_rho_press(rho_rc(f), press_rc(f), Ye_rc(f), rad_pref_rc[f]);
        temp_rc(f) =
            temp_from_rho_eps(rho_rc(f), eps_rc(f), Ye_rc(f), rad_pref_rc[f]);
      }
    }
    ...
    const vec<CCTK_REAL, 2> cs2_rc([&](int f) ARITH_INLINE {
      const CCTK_REAL cs =
          csnd_from_rho_temp(rho_rc(f), temp_rc(f), Ye_rc(f), rad_pref_rc[f]);
      return cs * cs;
    });
...
extern "C" void AsterX_Fluxes(CCTK_ARGUMENTS) {
  ...
  case eos_3param::RadIdealGas: {
    ...
    CalcFlux<0>(cctkGH, eos_3p_rad_ig, rec_var, reconstruction,
                reconstruction_LO, reconstruct_params, fluxtype);
    CalcFlux<1>(cctkGH, eos_3p_rad_ig, rec_var, reconstruction,
                reconstruction_LO, reconstruct_params, fluxtype);
    CalcFlux<2>(cctkGH, eos_3p_rad_ig, rec_var, reconstruction,
                reconstruction_LO, reconstruct_params, fluxtype);
    break;
  }
