...
enum class c2p_first_t { None, Noble, NobleRad, RePrimAnd, Palenzuela, Entropy };
enum class c2p_second_t { None, Noble, NobleRad, RePrimAnd, Palenzuela, Entropy };
...
apply_bh_interior_for_eos(const c2p_2DNoble &c2p_Noble,
	  ...
	  if constexpr (std::is_same_v<EOSType, EOSX::eos_3p_rad_idealgas>) {
	    c2p_Rad.bh_interior_rad<EOSType, limiting>(eos_3p, pv, cv, glo,
	                                               rad_pref);
	  } else {
...
solve_noble_rad_for_eos(const c2p_Noble_rad &c2p_Rad, const EOSType *eos_3p,
	                        ...
	                        const smat<CCTK_REAL, 3> &glo,
	                        const CCTK_REAL rad_pref, c2p_report &rep) {
	  if constexpr (std::is_same_v<EOSType, EOSX::eos_3p_rad_idealgas>) {
	    c2p_Rad.solve(eos_3p, pv, pv_seeds, cv, alp, beta, glo, rad_pref, rep);
	  } else {
	    assert(0);
	  }
}
...
void AsterX_Con2Prim_typeEoS(CCTK_ARGUMENTS, EOSIDType *eos_1p,
  ...
  } else if (CCTK_EQUALS(c2p_prime, "Noble_rad")) {
    c2p_fir = c2p_first_t::NobleRad;
  ...
  } else if (CCTK_EQUALS(c2p_second, "Noble_rad")) {
    c2p_sec = c2p_second_t::NobleRad;
  ...
  const auto c2p_impl = [=] CCTK_DEVICE(
    ...
    // Radiation prefactor: the only quantity derived from the local optical
    // depths; all EOS calls below depend on radiation solely through it.
    const EOSX::optical_depths od_local = local_optd<EOSType>(od_gfs, p.I);
    const CCTK_REAL rad_pref_local =
    ...
    c2p_Noble_rad c2p_Rad(eos_3p, atmo, max_iter, c2p_tol, alp_thresh, vw_lim,
                          B_lim, rho_BH, eps_BH, vwlim_BH, sigma_max,
                          inv_beta_max, Ye_lenient, use_z, use_temperature,
                          use_press_atmo, soft_root_convergence,
                          soft_root_width_factor);
    ...
    prim_vars pv_seeds{saved_rho(p.I),
                       saved_eps(p.I),
                       saved_Ye(p.I),
                       press_from_rho_eps(saved_rho(p.I), saved_eps(p.I),
                                          saved_Ye(p.I)),
                       temperature(p.I),
                       entropy_from_rho_eps(saved_rho(p.I), saved_eps(p.I),
                                            saved_Ye(p.I)),
                       v_up,
                       wlor,
                       Bup};
    ...
    if (excise) {
      ...
      if (mask_local != 1.0) {
	        apply_bh_interior_for_eos<EOSType, false>(
	            c2p_Noble, c2p_Rad, eos_3p, pv_seeds, cv, glo, rad_pref_local);
        pv = pv_seeds;
        call_c2p = false;
      }
    }
    ...
    if (call_c2p) {
      ...
      case c2p_first_t::NobleRad: {
	        solve_noble_rad_for_eos(c2p_Rad, eos_3p, pv, pv_seeds, cv, alp_avg,
	                                beta_avg, glo, rad_pref_local, rep_first);
        break;
      }
      ...
      case c2p_first_t::Palenzuela: {
        c2p_Pal.solve(eos_3p, pv, cv, alp_avg, beta_avg, glo, rad_pref_local,
                      rep_first);
        break;
      }
      ...
      if (rep_first.failed()) {
        ...
        case c2p_second_t::NobleRad: {
	          solve_noble_rad_for_eos(c2p_Rad, eos_3p, pv, pv_seeds, cv, alp_avg,
	                                  beta_avg, glo, rad_pref_local, rep_second);
          break;
        }
        ...
        case c2p_second_t::Palenzuela: {
          c2p_Pal.solve(eos_3p, pv, cv, alp_avg, beta_avg, glo,
        ...
        if (use_entropy_fix) {
          ...
          if (rep_ent.failed()) {
            ...
            if (mask_local != 1.0) {
              // Failure inside mask
	              apply_bh_interior_for_eos<EOSType, false>(
	                  c2p_Noble, c2p_Rad, eos_3p, pv_seeds, cv, glo,
	                  rad_pref_local);
              pv = pv_seeds;
            } else {
      ...
      if ((mask_local != 1.0) && c2p_flag_local) {
	        apply_bh_interior_for_eos<EOSType, true>(
	            c2p_Noble, c2p_Rad, eos_3p, pv, cv, glo, rad_pref_local);
      }
    }
...
extern "C" void AsterX_Con2Prim(CCTK_ARGUMENTS) {
  ...
  case eos_3param::RadIdealGas: {
    ...
    AsterX_Con2Prim_typeEoS(CCTK_PASS_CTOC, eos_1p_poly, eos_3p_rad_ig);
    break;
  }
